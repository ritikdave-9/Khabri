    ...
    public class NewsLikeDislikeController : ControllerBase
    {
        ...
        public async Task<IActionResult> Like([FromQuery] int userId, [FromQuery] int newsId)
        {
            try
            {
                ...
                CustomLogger.LogSuccess($"User {userId} liked news {newsId}.");
                return Ok(new { Message = "News liked." });
            }
            catch (Exception ex)
            {
                CustomLogger.LogError($"Error liking news: userId={userId}, newsId={newsId}. Details: {ex.Message}");
        ...
        public async Task<IActionResult> Dislike([FromQuery] int userId, [FromQuery] int newsId)
        {
            try
            {
                ...
                CustomLogger.LogSuccess($"User {userId} disliked news {newsId}.");
                return Ok(new ErrorResponseDto{ Message = "News disliked." });
            }
            catch (Exception ex)
            {
                CustomLogger.LogError($"Error disliking news: userId={userId}, newsId={newsId}. Details: {ex.Message}");
        ...
        public async Task<IActionResult> RemoveReaction([FromQuery] int userId, [FromQuery] int newsId)
        {
            try
            {
                ...
                CustomLogger.LogSuccess($"User {userId} removed reaction from news {newsId}.");
                return Ok(new ErrorResponseDto{ Message = "Reaction removed." });
            }
            catch (Exception ex)
            {
                CustomLogger.LogError($"Error removing reaction: userId={userId}, newsId={newsId}. Details: {ex.Message}");
        ...
        public async Task<IActionResult> GetCounts([FromQuery] int newsId)
        {
            try
            {
                ...
                CustomLogger.LogInformation($"Fetched like/dislike counts for newsId={newsId}: Likes={likes}, Dislikes={dislikes}");
                return Ok(new { Likes = likes, Dislikes = dislikes });
            }
            catch (Exception ex)
            {
                CustomLogger.LogError($"Error fetching like/dislike counts: newsId={newsId}. Details: {ex.Message}");
        ...
        public async Task<IActionResult> GetUserReaction([FromQuery] int userId, [FromQuery] int newsId)
        {
            try
            {
                ...
                CustomLogger.LogInformation($"Fetched user reaction: userId={userId}, newsId={newsId}, reaction={reactionStr}");
                return Ok(new { Reaction = reactionStr });
            }
            catch (Exception ex)
            {
                CustomLogger.LogError($"Error fetching user reaction: userId={userId}, newsId={newsId}. Details: {ex.Message}");
